...
<div class='block'>
    ...
    <div class="grid-view">
		...
		<table class="items">
			...
			<td>
				<?= CHtml::link('Отключить', array('/user/user/offOffer/id/' . $landing->land_id), array('class'=>'btn btn-primary')); ?>
			</td>
			<?php } ?>
			
			<?php if (!count($usersLandings) or !array_key_exists($landing->land_id, $usersLandings)) { ?>
			<td>
				<?= CHtml::link('Включить', array('/user/user/onOffer/id/' . $landing->land_id), array('class'=>'btn btn-primary')); ?>
			</td>
			<?php }?>
